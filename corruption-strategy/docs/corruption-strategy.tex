...
%	\subsection{Explicaci\'on del algoritmo}
	...
	\begin{algorithm}
		...
		{\textbf{end try}}
		\begin{algorithmic}[1]
			\Function{max\_flow\_min\_cut}{$G$}
			\State \textit{G\_r} = \textsc{get\_residual\_graph(\textit{G})}
			\State \textit{s} = \textit{'source'}
			\State \textit{t} = \textit{'sink'}
			
			\State \textit{p} = \textsc{find\_augmenting\_path(\textit{G\_r, s, t})}
			
			\While {\textit{p != None}}
			\State \textit{capacity\_p} = \textsc{residual\_capacity\_path(G\_r, p)}
			\For{\textit{(u, v) in p}}
			\try
			...
			\endtry
			\State \textit{ G\_r }= \textsc{get\_residual\_graph($G$)}
			\State \textit{p} = \textsc{find\_augmenting\_path($G\_r, s, t$)}
			\EndFor
			\EndWhile
			\State \textit{cities, roads, profit } = \textsc{get\_project\_distro(\textit{G\_r, G})}
			\State \textit{max\_flow} = 0
			
			\For{\textit{v in G.neighbors(s)}}
			\State \textit{max\_flow += G[s][v]['flow']}
			\EndFor
