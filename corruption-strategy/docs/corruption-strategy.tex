...
%
	\begin{lem}\label{flujo_red_r}
		Sea G=〈V,E〉 una red de flujo, sea f un flujo en G y sea p un camino aumentativo en la red residual $G_f$. Si se define el flujo $f_p$ , sobre la red residual, como
...
%	\end{col}
%	
	\begin{thm}\label{max_flow_min_cut}
	...
	El flujo m\'aximo se obtiene buscando en cada iteración del algoritmo un camino aumentativo [\ref{camino_aumentativo}]. Una vez encontrado el camino aumentativo, se aumenta el flujo en esa ruta tanto como sea posible. Este proceso se repite hasta que ya no hayan caminos aumentativos en el grafo residual [\ref{capacidad_residual}], ya que por teorema [\ref{max_flow_min_cut}], en ese momento se debe haber alcanzado el flujo m\'aximo.
