...
def backtrack_solution(n, m, a, s):
    ...
    _backtrack_solution(0, candidates, positions,
                        best_solution, 0, a, s, answer)
...
def _backtrack_solution(max, candidates, positions, best_solution, count, a, s, answer):
    ...
    for j in range(len(candidates)):
        if not candidates[j]:
            candidates[j] = True
            positions[count] = j
            _backtrack_solution(max + s[j][count] + a[j], candidates,
                                positions, best_solution, count + 1, a, s, answer)
            candidates[j] = False
